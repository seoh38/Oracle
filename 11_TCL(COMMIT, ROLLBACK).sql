...
<<<<<<< HEAD
        ...
        * 트랜잭션
          ...
          - 데이터의 변경사항을 묶어서 하나의 트랜잭션에 담아서 처리한다.
            (COMMIT 하기 전까지의 번경사항들을 하나의 트랜잭션에 담게 된다.)
          - 트랜잭션은 DML(INSERT, UPDATE, DELETE)을 대상으로 진행된다.
          - COMMIT(트랜잭션 종료 처리 후 저장), ROLLBACK(트랜잭션 취소), SAVEPOINT(임시저장)
          
            @COMMIT
               메모리 버퍼의 임시저장된 데이터를 DB에 반영
               모든 작업들을 정상적으로 처리하겠다고 확정하는 구문, 하나의 트랜잭션 과정을 종료하개 된다.
            @ROLLBACK
               메모리 버퍼에 임시저장된 데이터를 삭제한 후 마지막 COMMIT 시점으로 돌아간다.
               모든 작언들을 취소 처리하겠다고 확정하는 구문, 하나의 트랜잭션 과정을 취소하게 된다.
            @SAVEPOINT 
               저장점을 정의해두면 ROLLBACK 진행할 때 전체 작업을 ROLLBACK 하는게 아닌 SAVEPOINT 까지의 일부만 롤백한다.
...
SAVEPOINT SP; --Savepoint이(가) 생성되었습니다.

-- EMP_ID가 200번인 사원 지우기
DELETE FROM EMP_01
WHERE EMP_ID = 200;

ROLLBACK TO SP; -- 200번 사원이 지워지기 전으로 ROLLBACK

ROLLBACK; -- 900, 901 사원이 지워지기 전으로 ROLLBACK
=======
...
<<<<<<< HEAD
-- DDL 구문을 실행하는 순간 기존에 메모리 버퍼에 임시저장된 변경사항들이 무조건 DB에 반영된다.(COMMIT 시켜버린다.)
CREATE TABLE TEST(
   TID NUMBER
);

ROLLBACK; -- 213번 사원이 돌아오지 않는다. WHY? 
=======
