...
<<<<<<< HEAD
       PL/SQL 문을 저장하는 객체
       필요할때 마다 복잡한 구문을 다시 입력할 필요가 없이 간단하게 호출해서 실행결과를 얻을수 있다.
...
>>>>>>> 8cdd04c0d5b88dfef0b20461f14a9496aa826f2f
-- 테스트용 테이블 생성
...
-- SOURCE : 프로시저를 관리하는 데이터 딕셔너리
SELECT * FROM USER_SOURCE; -- 프로시저 작성 구문이 한 행씩 저장되어있다.
...
BEGIN
    ...
    FROM EMPLOYEE
    WHERE EMP_ID = V_EMP_ID; --> 매개변수로 지정
END;
/

-- 직접 변수를 DBMS에 선언(바인드 변수 : VARIABLE, VAR)
-- 프로시저를 실행시 조회결과를 받아오기 위해 바인드 변수를 생성해야한다.
=======
SELECT * FROM EMPLOYEE;
...
<<<<<<< HEAD
-- 외부에서 받아온 값을 변수안에 담아줄수 있다. 참조값을 넘겨준다.
EXECUTE SELECT_EMP_ID('200',  :VAR_EMP_NAME, :VAR_SALARY, :VAR_BONUS);
=======
...
PRINT VAR_BONUS;
<<<<<<< HEAD
--> 바인드 변수의 값을 자동으로 호출해주는 설정값
SET AUTOPRINT ON; 

EXECUTE SELECT_EMP_ID('201',  :VAR_EMP_NAME, :VAR_SALARY, :VAR_BONUS);

----------------------------------------------------------------------------
/*
    <FUNCTION>
       프로시저와 사용용도가 거의 비슷하지만 
...
RETURN NUMBER
IS
    V_SAL EMPLOYEE.SALARY%TYPE; --> 변수 생성
=======
    V_EMP_ID EMPLOYEE.EMP_ID%TYPE
)
RETURN NUMBER
...
<<<<<<< HEAD
    RETURN (V_SAL + (V_SAL * V_BONUS)) * 12; --> 결과값을 리턴
=======
...
-- 만들어 놓은 FUNCTION(BONUS_CALC)을 사용해서 
...
/*
    <CURSOR>
       SQL문의 처리 결과(처리 결과가 여러 개의 행(ROW))을 담고 있는 객체이다.
       커서 사용시 여러 행으로 나ㅏ난 처리 결과에 순차적으로 접근이 가능하다.
       
       * 커서 종류
        묵시적/명시적 커서 두 종류가 존재한다.
        
       * 커서 속성(묵시적 커서의 경우 커서명은 SQL로 시작)
        ...
        1) 묵시적 커서
         ...
         - PL/SQL 블록에서 SQL문을 실행 시 마다 자동으로 만들어져서 사용된다.
         - 시용자는 생성 유무를 알 수 없지만, 커서 속성을 활용하여 커서의 정보를 얻어올 수 있다.
*/
...
DECLARE
    ...
    CURSOR C1 IS -- (커서 선언부분에 서브쿼리를 넣어준다)
...
BEGIN
    ...
    LOOP
        -- 서브 쿼리 결과에서 한 행씩 차례대로 데이터를 가져온다.
...
--       SELECT * FROM DEPARTMENT;       
BEGIN
--   LOOP 시작시 자동으로 커서를 생성하고 커서를 OPEN한다.
--   반복할 때마다 FETCH도 자동으로 실행된다.
--   LOOP 종료시 자동으로 커서가 CLOSE 된다.
--   FOR V_DEPT IN C1
...
*/
-- 1) V패키지 선언부에 변수, 상수 선언 및 사용법
CREATE OR REPLACE PACKAGE TEST_PACKAGE
IS
   NAME VARCHAR2(20); -- 변수
   PI CONSTANT NUMBER := 3.12; -- 상수
END;
/
...
END;
/
-- 에러발생, 패키지 BODY 부분을 생성해야 한다.
-- "not executed, %s does not exist"
