    ...
    <UPDATE>
        ...
        [표현법]
            ...
            SET 컬럼명 = 변경하려는 값,
                컬럼명 = 변경하려는 값,
                ...
            [WHERE 조건]; -- 조건을 작성하지 않으면 모든 컬럼이 변경된다.
...
-- 2) 다중열 서브쿼리를 사용해서 SALARY와 BONUS 컬럼을 한번에 변경(쌍으로 묶어줌)
UPDATE EMP_SALARY
SET (SALARY, BONUS) = (
     SELECT SALARY,BONUS
      FROM EMP_SALARY
...
-- 노옹철, 전형동, 정중하, 하동운 사원들의 급여와 보너스를 유재식 사원과 동일하게 변경
UPDATE EMP_SALARY
SET (SALARY, BONUS) = (
     SELECT SALARY,BONUS
      FROM EMP_SALARY
...
-- EMP_SALARY 테이블에서 아시아 지역에 근무하는 직원의 보너스를 0.3으로 변경(JOIN)
SELECT E.EMP_ID
FROM EMPLOYEE E
...
-- DEPARTMENT 테이블에서 DEPT_ID가 'D1' 인 부서를 삭제
DELETE FROM DEPARTMENT
WHERE DEPT_ID = 'D1'; -- child record found 자식 테이블에서 참조하고 있으면 삭제를 할 수 없다.
-- D1의 값을 참조하는 자식 테이블의 데이터가 있기 때문에 삭제가 되지 않는다.

-- DEPARTMENT 테이블에서 DEPT_ID가 'D3' 인 부서를 삭제
DELETE FROM DEPARTMENT
WHERE DEPT_ID = 'D3'; -- 자식테이블에서 참조하고 있지 않는 값이면 삭제가 가능하다.

/*
    <TRUNCATE>
        테이블의 전체 행을 삭제할 때 사용하는 구문으로 DELETE 보다 수행 속도가 더 빠르다.
        별도의 조건제시가 불가능하고 ROLLBACK이 불가능하다.
...
TRUNCATE TABLE EMP_SALARY; -- Table EMP_SALARY이(가) 잘렸습니다.
                           -- 물리적으로 차지하고 있는 공간까지도 지워준다.
ROLLBACK; --> TRUNCATE 사용시 ROLLBACK  불가능하다.
=======
