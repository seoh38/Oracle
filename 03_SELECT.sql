...
-- 아래와 같이 쿼리는 대소문자를 가리지는 않지만 관례상 대문자로 작성한다.
select emp_id, emp_name, salary
from employee;

------------------ 실습문제 ----------------
-- 1. JOB 테이블의 모든 컬럽 정보 조회
SELECT *
FROM JOB; 

-- 2. JOB 테이블의 직급명 컬럼 정보 조회
SELECT JOB_NAME
FROM JOB;
...
-- 4. EMPLOYEE 테이블의 사원명, 이메일 전화번호 입사일 정보 조회
...
-- EMPLOYEE 데이블에서 직원명 월급여 연봉 보너스가 포함된 연봉((급여 + ( 보너스 + 급여 ))  *12) 조회
-- 산술연상 중 NULL값이 존재할 경우 산술 연산한 결과값은 무조건 NULL이다.
...
-- DATE형식끼리도 연산이 가능하다.
-- SYSDATE은 현재날짜를 출력한다.
-- FLOOR(NUMBER)은 소수점을 버리는 함수이다.
-- CELL(NUMBER)은 매개값으로 전달되는 수를 올림하는 함수
...
/*
  <컬럼명의 별칭 지정하기>
    [표현법]
     ...
     - 산술연산을 하게 되면 컬럼명이 지저분해진다 이때 컬렴명의 별칭을 부여해서 깜끔하게 보여줄 수 있다.
     - 별칭을 부여할때 띄어쓰기 똔ㄴ 특수문자가 별칭에 포함될 경우 반드시 큰따옴표(" ")로 감싸주어야 한다.
*/
-- EMPLOYEE 테이블에서 직원명, 급여, 연봉 보너스가 포함된 연봉
...
*/
-- EMPLOYEE 테이블에서 직원번호 직원명 급여 단위(원)조회
...
-- EMPLOYEE 테이블에서 직원 코드(중복 제거) 조회
SELECT DISTINCT JOB_CODE
FROM EMPLOYEE;

-- DISTINCT는 SELECT절에서 한번만 구현할 수 있다.
-- SELECT DISTINCT JOB_CODE, DISTINCTDEPT_CODE
-- 칼럼을 두개를 조회할때 둘 다 모두 같은 값을 중복제거해준다.
...
-- EMPLOYEE 테이블에서 부서코드가 D9와 일치하는 사원들의 모든 칼럼 조회
...
-- EMPLOYEE 테이블에서 부서코드가 D9가 아닌 사원들의 사번, 사원명 부서코드 조회
...
-- EMPLOYEE 테이블에서 급여가 400만원 이상인 직원들의 직원명 부서코드 조회
...
-- EMPLOYEE 테이블에서 재직중(ENT_YN 컬럼의 값이 'N')인 직원들의 사번 이름 입사일 조회
...
-- 2. EMPLOYEE 테이블에서 연봉이 5000만원 이상인 직원의 직월명 급여 연봉 입사일 조회
SELECT EMP_NAME, SALARY, HIRE_DATE
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 이름에 '하'가 포함된 사원의 사원명, 주민번호, 부서 코드 조회
...
FROM EMPLOYEE
WHERE PHONE LIKE '___9%'; -- %를 넣어야 ***9 로 시작하는 전화번호가 들어간 모든 행 조회 가능. '___9' 는 4자리 수 컬럼만 조회 가능.

-- EMPLOYEE 테이블에서 이메일 중 '_'라는 문자의 앞 글자가 3자리인 이메일을 가진 사원의 사번, 사원명, 이메일 조회
SELECT EMP_ID, EMP_NAME, EMAIL
FROM EMPLOYEE
--WHERE EMAIL LIKE '____%'; -- 와일드카드와 데이터 값이 구분되지 않는다
--WHERE EMAIL LIKE '___\_%'; -- \가 문자로 인식되어 조회되지 않는다.
WHERE EMAIL LIKE '___!_%' ESCAPE '!'; -- \뒤의 _는 와일드카드가 아닌 일반 문자의 역할로 쓰인다. (꼭 \일 필요없다. 임의의 문자면 모두 가능)
                                      -- 데이터로 처리할 값 앞에 임의의 문자를 제시하고 임의의 문자는 ESCAPE 옵션에 등록한다.
...
--WHERE NOT EMP_NAME NOT LIKE '김%'; -- 부정의 부정
...
-- 2. EMPLOYEE 테이블에서 전화번호 처음 3자리가 010이 아닌 사원의 이름, 전화번호
...
-- EMPLOYEE 테이블에서 관리자(사수)가 없는 (+부서도 없는) 사원의 이름, 부서코드 조회
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
--WHERE MANAGER_ID = NULL; -- NULL은 비교연산자를 사용할 수 없다.
WHERE MANAGER_ID IS NULL AND DEPT_CODE IS NULL;

-- EMPLOYEE 테이블에서 부서 배치를 받지 않았지만, 보너스는 받는 사원의 사원명, 부서 코드, 보너스 조회
...
-- EMPLOYEE 테이블에서 D5,D6,D8 부서원들의 사원명, 부서 코드, 급여 조회
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
--WHERE DEPT_CODE = 'D5' OR DEPT_CODE = 'D6' OR DEPT_CODE = 'D8'; -- IN으로 간결하게 표현할 수 있다.
...
SELECT EMP_ID || EMP_NAME || SALARY AS "급여정보" -- 컬럼이 하나처럼 연결되어 출력된다. 
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 사원명, 급여를 리터럴과 연결해서 조회
SELECT EMP_NAME || '의 월급은 ' || SALARY || '원 입니다.' AS "급여정보"-- 문자열에도 ||를 넣어 컬럼이 하나처럼 연결되게 출력된다. 
FROM EMPLOYEE;
---------------------------------------------------
...
-- EMPLOYEE 테이블에서 BONUS로 오름차순으로 정렬
SELECT *
FROM EMPLOYEE
--ORDER BY BONUS ASC; -- 오름차순 정렬은 기본적으로 NULLS LAST
ORDER BY BONUS ASC NULLS FIRST;

-- EMPLOYEE 테이블에서 BONUS로 내림차순으로 정렬
-- (단, BONUS 값이 일치할 경우 그때는 SALARY가지고 오름차순으로 정렬
SELECT *
FROM EMPLOYEE
--ORDER BY BONUS DESC; -- 내림차순 정렬은 기본적으로 NULLS FIRST 
ORDER BY BONUS DESC NULLS LAST, SALARY ASC;

-- EMPLOYEE 테이블에서 연봉별 내림차순으로 정렬된 사원의 사원명, 연봉조회
---------------------------------------------------
/*
    <IN>
      [표현법]
